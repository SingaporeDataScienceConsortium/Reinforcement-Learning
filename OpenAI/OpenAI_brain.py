...
class DeepQNetwork:
    def __init__(
            ...
            batch_size=32, # a batch stored information
        ...
        self.memory = np.zeros((self.memory_size, n_features * 2 + 2)) # size = [500, 4*2+2]
    ...
    def build_net(self):
        # ------------------ build evaluate_net ------------------
        self.s = tf.compat.v1.placeholder(tf.float32, [None, self.n_features])  # input: a list of features
        self.q_target = tf.compat.v1.placeholder(tf.float32, [None, self.n_actions])  # used to calculate loss later
    ...
    def store_transition(self, s, a, r, s_):
        ...
        transition = np.hstack((s, a, r, s_)) # a vector of 10 elements
